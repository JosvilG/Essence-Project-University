...
public class ActivaSpawn : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if(this.name== "triggerEnemicsPlatja")
        {
            camaraNormal.SetActive(false);
            camaraCombat.SetActive(true);
        }

        if (this.name == "triggerMostraOrbe" && potEscena==true && other.tag=="Player" && posicioAlMon.orbe1==false)
        {
            escenaCamara();
        }

        /*Aquests 2 nomes per la illa inicial*/
        if (this.name == "colisioEspasa" && potEscena == true && AttackControl.getEspasa == false)
        {
            camaraNormal.SetActive(false);
            camaraCombat.SetActive(true);
        }

        if (this.name == "colisioEspasa" && potEscena == true && AttackControl.getEspasa == true)
        {
            camaraNormal.SetActive(true);
            camaraCombat.SetActive(false);
        }

        if (this.name == "colisioEspasaSurt" && potEscena == true && AttackControl.getEspasa==true)
        {
            camaraNormal.SetActive(true);
            camaraCombat.SetActive(false);
        }

        if (this.name == "colisioEspasaSurt" && potEscena == true && AttackControl.getEspasa == false)
        {
            camaraNormal.SetActive(true);
            camaraCombat.SetActive(false);
        }
