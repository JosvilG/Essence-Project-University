...
public class Boomerang : MonoBehaviour // original
{
    bool go;
    ...
    void Start()
    {
       ...
       //boomerang.GetComponent<MeshRenderer>().enabled = false;  //Amaga l'arma actual
        ...
        locationInFrontOfPlayer = new Vector3(player.transform.position.x, player.transform.position.y, player.transform.position.z) + player.transform.forward * 15f;
    ...
    void Update()
    {
        itemToRotate.transform.Rotate(0, Time.deltaTime * 300, 0);
        if (go)
        {
